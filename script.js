...
var upRight, upLeft, downLeft, downRight;  //moves allowed to each piece
...
function showMoves(piece) {
    ...
    // if no match was found; it happens when, for example, white moves and you press black
    if (!match) {
        return 0;
    }
    ...
    /*if i cant attack , i check if i can move away*/
...
function makeMove(index) {
    var isMove = false;
    if (!selectedPiece)   //if the game has started and no track has been selected
        return false;
    ...
    if (index == cpy_upRight) {
        ...
        if (the_checker[1].color == "white") {
            ...
            //remove the track if a jump was executed
...
function executeMove(X, Y, nSquare) {
    // change the coordinates of the moved piecee
    the_checker[selectedPieceindex].changeCoord(X, Y);
    the_checker[selectedPieceindex].setCoord(0, 0);
    // I release the field occupied by the piece and occupy the one it will occupy
    block[the_checker[selectedPieceindex].ocupied_square].ocupied = false;
    //alert (the_checker[selectedPieceindex]. occupied_square);
